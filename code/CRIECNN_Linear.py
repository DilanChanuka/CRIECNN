...
import sys
sys.path.insert(1, '/content/drive/MyDrive/CRIECNN/code/encode_for_linear_RNAs/')

from Doc2Vec_Encoder_2 import Doc2Vec_Embedding
from Kmer_Encoder_2 import RNA_Kmer 
from BERT_Encoder_2 import RNABert
from EIIP_Encoder_2 import RNA_EIIP
...
import random
from protein_list import linearRNA_RBPs
import time
...
def CRIECNN(args):
    ...
    for train_index, eval_index in kf.split(train_label):
        ...
        model = createECNN(embedding_matrix, 101)
        ...
        print(np.shape(test_profile))
        print(np.shape(test_property))
        print(np.shape(test_main))
